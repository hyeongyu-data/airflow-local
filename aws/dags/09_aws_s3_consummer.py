...
- 렌터카 반납 -> 개인 촬영 -> 업로드 -> s3 -> 트리거작동 -> 데이터추출 전처리 -> AI모델 전달 -> 추론 -> 평가 -> 피해액x원 응답
    - 사용자가 언제 이런 행위를 할지 아무도 모름 -> 의외성 -> 소카모델 확인
'''
# 1. 모듈 가져오기
...
with DAG(
    ...
    # 최소 스케줄은 필요함 -> None X
    schedule_interval = '@daily', # DAG는 활성화 정도만 구성, 센서 작동에 스케줄이 필요한지 테스트
