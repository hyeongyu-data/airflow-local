...
with DAG(
    ...
    t1 = S3DeleteObjectsOperator(
        ...
        prefix      = f's3://{BUCKET_NAME}/', # 해당 위치가 대상
    ...
    # csv -> 테이블 매핑 -> 쿼리수행 -> 결과를 저장 (필요시 포맷 변환)
    # 테스트 응시 결과가 90점 이상 학생만 추출 결과를 담는 테이블 -> TARGET_TABLE
    # PARQUET : 압축형태 지원, GZIP등 포맷사용, 열기반 데이터 관리
    ...
    t3 = AthenaOperator(
        ...
        do_xcom_push    = True # 테이블 만들어졌나? 센서 가동 조건응로 xcom 활용
        # xcom 을 통해서 TARGET_TABLE이 생성되었는지 체크 -> 확인 -> t4내 기타 처리등 활용
    )
    # CTAS 
    # 10분간 최대 대기, 10초 간격 감시 -> create_table_format_parquet 테스크가 완료되었는지 점검
    # athena상에 테이블이 완성되었는지 감시
    t4 = AthenaSensor(
        task_id = 'sensor',
        # 앞 테스크를 감시
        query_execution_id  = "{{ task_instance.xcom_pull(task_id='create_table_format_parquet')}}",
